...
class FeatureContext extends BehatContext implements ClosuredContextInterface {
	...
	public function get_media_library_count() {
		$result = $this->proc( 'wp post list --post_type=attachment --format=count' )->run_check();

		return (int) $result->stdout;
	}

	public function get_uploads_file_count() {
		$result = $this->proc( 'find wp-content/uploads -type f | wc -l' )->run_check();
	 ...
	 * @return Finder
	 */
	protected function find_in_uploads() {
		return Finder::create()->in("{$this->variables['RUN_DIR']}/wp-content/uploads/");
	}

	public function find_uploads_containing_matching_images($args = []) {
		return $this->find_in_uploads()
		              ->files()
		              ->filter(function(\Symfony\Component\Finder\SplFileInfo $file) use ($args) {
							$filepath = $file->getRealPath();

							if (0 !== strpos(mime_content_type($filepath), 'image/')) {
								return false;
							}
							...
							if (isset($args['ext']) && $file->getExtension() != $args['ext']) {
								return false;
							}
