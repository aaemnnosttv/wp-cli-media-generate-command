...
$steps->Given('/^the media library should contain (\d+) items$/', function($world, $expected_count) {
	$actual_count = $world->get_media_library_count();

	if ((int) $actual_count !== (int) $expected_count) {
		throw new Exception("The media library was expected to have $expected_count items, but actually contains $actual_count.");
	}
});

$steps->Given('/^the uploads directory should contain (\d+) files?$/', function($world, $expected_count) {
	$actual_count = $world->get_uploads_file_count();

	if ((int) $actual_count !== (int) $expected_count) {
		throw new Exception("The uploads directory was expected to have $expected_count files, but actually contains $actual_count.");
	}
});

$steps->Then('/^the uploads directory should contain (\d+) images? (\d+)px wide and (\d+)px tall$/', function($world, $count, $width, $height) {
	$found = $world->find_uploads_containing_matching_images(compact('width','height'))->count();

	if ((int) $found !== (int) $count) {
		throw new Exception("Expected to find $count images {$width}x{$height} but found $found.");
	}
});

$steps->Then('/^the uploads directory should contain (\d+) (jpe?g|png|gif)$/', function($world, $count, $ext) {
	$found = $world->find_uploads_containing_matching_images(compact('ext'))->count();

	if ((int) $found !== (int) $count) {
		throw new Exception("Expected to find $count images with a $ext extension but found $found.");
	}
});
